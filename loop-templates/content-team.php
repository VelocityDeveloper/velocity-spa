<?php
/**
 * Post rendering content according to caller of get_template_part.
 *
 * @package velocity
 */
?>
<article <?php post_class('col-md-3 col-6 container border-0 mb-3 px-2'); ?> id="post-<?php the_ID(); ?>">
    <div class="card h-100 border-0 rounded-4 shadow-sm p-2">
	<div class="overflow-hidden rounded-4"><?php velocity_spa_post_thumbnail(get_the_ID(), 'ratio-1x1'); ?></div>
		<h2 class="h5 fw-bold text-center p-3 mb-0"><a class="text-dark" href="<?php echo esc_url(get_permalink()); ?>" rel="bookmark"><?php echo esc_html(wp_trim_words(get_the_title(), 5)); ?></a></h2>
    </div>
</article><!-- #post-## -->
